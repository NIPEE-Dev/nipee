...
if (!function_exists('journeyText')) {
    function journeyText($workingDay): string
    {
        ...
        $startHour = substr($workingDay->start_hour, 0, 5);
        $endHours = substr($workingDay->end_hour, 0, 5);

        $text = "A Formação em Contexto de Trabalho decorrerá sempre que possível, no regime de {$weekdays[$workingDay->start_weekday]} à {$weekdays[$workingDay->end_weekday]} das {$startHour} às {$endHours}";
