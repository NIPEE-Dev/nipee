  ...
  useEffect(() => {
    ...
    const fetchLinkedCompany = async () => {
      ...
      try {
        ...
        console.error("Erro ao carregar empresa vinculada:", error);
        if (isMounted) {
          setLinkedCompany(null);
        }
      } finally {
        ...
        <Form>
                ...
                {({ records, isLoading }) => {
                  React.useEffect(() => {
                    if (
                      isBranchOrSectorRole &&
                      linkedCompany?.id &&
                      formProps.values.company_id !== linkedCompany.id
                    ) {
                      formProps.setFieldValue("company_id", linkedCompany.id);
                      return;
                    }
                    ...
                    if (
                      ...
                      records.length > 0 &&
                      !isAdmin &&
                      !formProps.values.company_id
                    ) {
                      formProps.setFieldValue("company_id", records[0].id);
                    }
                  }, [
          ...
          >
            <FormControl as="fieldset" mb={6}>
              <RadioGroup
                onChange={(val) => {
                  formProps.setFieldValue("working_day.schedule_type", val);
                  if (val === "flexible" && !formProps.values.working_day?.flexible_text) {
                    formProps.setFieldValue(
                ...
                value={formProps.values.working_day?.schedule_type || "fixed"}
              >
            ...
            {formProps.values.working_day?.schedule_type !== "flexible" && (
              <>
                <Stack direction={["column", "row"]} spacing="24px">
                  <FastField
